...
def main(cfg: PretrainActionPriorConfig):
    ...
    # Instantiate the full head, but only optimize action_prior
    ...
    # Only train action_prior parameters
    for name, param in action_head.named_parameters():
        param.requires_grad = name.startswith("action_prior")

    trainable_params = [p for p in action_head.parameters() if p.requires_grad]
    print(f"# trainable params in action_prior: {sum(p.numel() for p in trainable_params)}")
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as pbar:
        while step < cfg.max_steps:
            for batch in dataloader:
                ...
                out = action_head.pretrain_action_prior(
                    gt_actions,
                ...
                if distributed_state.is_main_process and step > 0 and step % cfg.save_freq == 0:
                    ckpt_path = run_dir / f"action_prior--{step}_checkpoint.pt"
                    torch.save(action_head.action_prior.state_dict(), ckpt_path)
                    print(f"Saved action_prior checkpoint to {ckpt_path}")
    ...
    if distributed_state.is_main_process:
        final_ckpt = run_dir / "action_prior--final_checkpoint.pt"
        torch.save(action_head.action_prior.state_dict(), final_ckpt)
        print(f"Saved final action_prior checkpoint to {final_ckpt}")
