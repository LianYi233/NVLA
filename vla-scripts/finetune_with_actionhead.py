"""
pretrain action head
finetune.py
Fine-tunes Qwen2.5-0.5B via LoRA.


run:
CUDA_VISIBLE_DEVICES=0 python vla-scripts/pretrain_action_prior.py   --data_root_dir data/libero   --dataset_name libero_object_no_noops   --run_root_dir /data/wyn/chkpt/action_prior   --batch_size 64   --learning_rate 1e-4   --max_steps 50000   --save_freq 5000   --num_steps_before_decay 50000   --hidden_dim 4096   --use_pro_version True   --noise_std 0.01   --lambda_smooth 0.01
...
print("action_heads imported from:", ah.__file__)
print("L1RegressionActionHead signature:", inspect.signature(L1RegressionActionHead.__init__))
...
class FinetuneConfig:
    ...
    ## new: about path planner
    use_path_planner: bool = False
    num_path_candidates: int = 8
    ...
    lambda_smooth: float = 0.001        # default 0.01 is too strong for chunk action
    ...
    # Logging
    wandb_entity: str = "LianYi233"  # Name of WandB entity
    wandb_project: str = "nvla-action_prior"  # Name of WandB project
...
def run_forward_pass(
    ...
    # Compute metrics for continuous action representations (L1 regression)
        ...
        if cfg.use_path_planner:
            head_out = action_head.module.predict_action(
                multi_layer_hidden_states,
                proprio=batch["proprio"] if use_proprio else None,
                proprio_projector=proprio_projector if use_proprio else None,
                phase=cfg.phase,
        ...
        if cfg.use_path_planner:
            predicted_actions = head_out["actions"]
            bc_loss = torch.nn.L1Loss()(predicted_actions, ground_truth_actions)
            plan_loss = head_out.get("plan_loss", 0.0)
            energy_loss = head_out.get("energy_loss", 0.0)

            smooth_loss = ((predicted_actions[:, 1:] - predicted_actions[:, :-1]) ** 2).mean() \
            if predicted_actions.shape[1] > 1 else 0.0

            loss = (
                bc_loss + cfg.lambda_plan * plan_loss + cfg.lambda_energy * energy_loss + cfg.lambda_smooth * smooth_loss
            )
        else:
            predicted_actions = head_out
            loss = torch.nn.L1Loss()(predicted_actions, ground_truth_actions)
        ...
        if cfg.use_path_planner:
            metrics.update({
                "plan_loss": float(plan_loss.item() if torch.is_tensor(plan_loss) else plan_loss),
                "energy_loss": float(energy_loss.item() if torch.is_tensor(energy_loss) else energy_loss),
                "smooth_loss": float(smooth_loss.item() if torch.is_tensor(smooth_loss) else smooth_loss),
            })
        ...
        if should_log_l1_loss:
            ...
            if compute_diffusion_l1:
                ...
                with open('./logs/current_actions_L1_loss.txt', 'a+') as f:
...
def save_training_checkpoint(
    ...
    if distributed_state.is_main_process:
        ...
        print("---saving_model---", log_step, checkpoint_dir)
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    if cfg.use_proprio:
        ...
        if cfg.use_l1_regression:
            if cfg.use_path_planner:
                head_cls = PathPlannerActionHead
                head_args = {
                    "input_dim": vla.module.llm_dim,
                    "hidden_dim": vla.module.llm_dim,
                    "action_dim": ACTION_DIM,
                    "use_pro_version": cfg.use_pro_version,
                    "num_candidates": cfg.num_path_candidates,
                ...
                head_args = {
                    ...
                    "action_prior_hidden_dim": cfg.action_prior_hidden_dim,
                }
    ...
    recent_metrics = {
        ...
        "plan_loss": deque(maxlen=cfg.grad_accumulation_steps),       ## for path planner
        "energy_loss": deque(maxlen=cfg.grad_accumulation_steps),     ## for path planner
        "smooth_loss": deque(maxlen=cfg.grad_accumulation_steps),     ## for path planner
    }
