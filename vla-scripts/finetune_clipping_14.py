## ----- adaptive version of clipping -----
## ----- 2025-DEC-31 -----
## run this:
##  CUDA_VISIBLE_DEVICES=0 torchrun --standalone --nnodes 1 --nproc_per_node 1 vla-scripts/finetune_clipping_d.py --vlm_path pretrained_models/prism-qwen25-extra-dinosiglip-224px-0_5b --config_file_path pretrained_models/configs --data_root_dir data/libero --dataset_name libero_object_no_noops --run_root_dir outputs --use_film False --num_images_in_input 2 --use_proprio True --use_lora True --use_fz False --use_minivlm True --image_aug True --num_steps_before_decay 200000 --max_steps 200005 --save_latest_checkpoint_only False --merge_lora_during_training True --batch_size 4 --grad_accumulation_steps 4 --learning_rate 2e-4 --lora_rank 64 --use_pro_version True

import sys
sys.path.append("../..")
sys.path.append("/home/hmq/wyn/VLA-Adapter/experiments/")
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    # ==================== ## 为VLM和action expert分别设置参数组和学习率 ====================
    # 分离VLM参数和action expert参数
    vlm_params = []
    ...
    # 添加参数量统计变量
    vlm_total_param_count = 0  # VLM总参数量（包含所有部分）
    vlm_trainable_param_count = 0  # VLM可训练参数量
    action_expert_param_count = 0  # Action expert参数量（应该是可训练的）

    print("----- watch params -----")
    # VLA中可训练的参数（主要是LoRA参数和action_queries）
    for name, param in vla.named_parameters():
        # 统计VLM总参数量（无论是否可训练）
        if "action_queries" not in name:
            vlm_total_param_count += param.numel()

        # 统计可训练参数
        if param.requires_grad:
            if "action_queries" in name:
                action_expert_params.append(param)
                action_expert_param_count += param.numel()
                print(f"isaction_queries__Action expert param: {name}, shape: {param.shape}")
            else:
                vlm_params.append(param)
                vlm_trainable_param_count += param.numel()
                print(f"noaction_queries__VLM param: {name}, shape: {param.shape}")
    ...
    if cfg.use_l1_regression:
        for name, param in action_head.named_parameters():
            if param.requires_grad:
                action_expert_params.append(param)
                action_expert_param_count += param.numel()
                print(f"Action head param: {name}, shape: {param.shape}")
    ...
    if cfg.use_proprio:
        for name, param in proprio_projector.named_parameters():
            if param.requires_grad:
                action_expert_params.append(param)
                action_expert_param_count += param.numel()
                print(f"Proprio projector param: {name}, shape: {param.shape}")

    # 打印详细的参数量统计
    print("\n" + "=" * 80)
    print("详细的参数量统计:")
    print("=" * 80)
    print(f"VLM 总参数量: {vlm_total_param_count:,} (包含DINO、Siglip、LLM等所有部分)")
    print(f"VLM 可训练参数量: {vlm_trainable_param_count:,} (LoRA参数等)")
    print(f"VLM 可训练参数比例: {vlm_trainable_param_count / vlm_total_param_count * 100:.2f}%")
    print(f"Action expert 参数量: {action_expert_param_count:,} (应该是可训练的)")
    print(f"总可训练参数量: {vlm_trainable_param_count + action_expert_param_count:,}")
    print(
        f"参数比例 (Action Expert / VLM可训练参数): {action_expert_param_count / (vlm_trainable_param_count + 1e-8):.4f}")
    print(f"参数比例 (Action Expert / VLM总参数): {action_expert_param_count / (vlm_total_param_count + 1e-8):.4f}")
    print("=" * 80 + "-------\n\n\n\n\n")
    ...
    for name, param in vla.named_parameters():
        if param.requires_grad:
            ...
            if "action_queries" in name:
                action_expert_params.append(param)
                print(f"isaction_queries__Action expert param: {name}")
            else:
                vlm_params.append(param)
                print(f"noaction_queries__VLM param: {name}")
    ...
    if cfg.use_l1_regression:
        for name, param in action_head.named_parameters():
            if param.requires_grad:
                action_expert_params.append(param)
                print(f"Action head param: {name}")
    ...
    if cfg.use_proprio:
        for name, param in proprio_projector.named_parameters():
            if param.requires_grad:
                action_expert_params.append(param)
                print(f"Proprio projector param: {name}")
    ...
    param_groups = [
        ...
        {'params': action_expert_params, 'lr': cfg.action_expert_learning_rate}
    ]
    ...
    original_lr_action = optimizer.param_groups[1]["lr"]
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if (batch_idx + 1) % cfg.grad_accumulation_steps == 0:
                ...
                if action_expert_params:
                    ...
                    print('vlm_grad_norm: ', vlm_grad_norm)
                    print('action_expert_grad_norm: ', action_expert_grad_norm)
